...
def run():
    ...
    for line in sys.stdin:
        ...
        #print(normalized)
        if status == True:
            values = [data["tweet_id"], data["created_at"], data["user_id"],
                #data["censored_text"],
                      json.dumps(data["censored_text"]),
                      data["impactScore"], data["TF"]]
            output = "\t".join(values)
...
def calculate_TF(total_cnt, word_list):
    word_dict = dict()
...
def set_filters(line):
    ...
    total_cnt, word_list = extract_words(text_lower)
    data["impactScore"] = str(calculate_impact_score(line, word_list))
    data["TF"] = calculate_TF(total_cnt, word_list)

    return True, data
    #return True, json.dumps(OrderedDict([("tweet_id", data["tweet_id"]), ("created_at", data["created_at"]),
    #    ("user_id", data["user_id"]), ("censored_text", data["censored_text"]),
    #    ("impactScore", data["impactScore"]), ("TF", data["TF"])]),
